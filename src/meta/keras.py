...
from src.utils.file_utils import EXTENSION_H5, EXTENSION_PB, MODELS_DIR
...
class KerasModel(AbstractTensorModel):
    ...
    def save(self, *argv, **kwargs):
        ...
        return _KerasModelSaver(model=self).save(filepath, **kwargs)
    ...
    def save_weights(self, *argv, **kwargs):
        ...
        return _KerasModelSaver(model=self, filepath=self.filepath_h5()).save_weights(filepath, **kwargs)
    ...
    def freeze_graph(self, dir_=None, keep_var_names=None, output_names=None, clear_devices=True):
        """
        ...
        outputs are removed.
        @param dir_ Override directory to freeze graph to
        ...
        with graph.as_default():
            ...
            logger.debug('%s Saving frozen graph to %s...', self.log_prefix(), self.filepath_pb())
            try:
                tf.io.write_graph(frozen_graph, dir_ or self.file_dir(self.metadata.epoch),
...
class _KerasModelSaver(AbstractTensorModelSaver):
    ...
    def __init__(self, model=None, filepath=None):
        ...
        self._model_h5 = filepath
        if self._model_h5 is not None:
            assert os.path.exists(self._model_h5)

        # XOR
        # this effectively determines the mode that we will be able to save to later
        #assert bool(self._model) != bool(self._model_h5)
    ...
    def save(self, filepath, representative_data=None, **kwargs):
        ...
        if extension == EXTENSION_H5:
            ...
            ret = self._save_model_pb(filepath, representative_data, **kwargs)
        else:
    ...
    def _save_model_pb(self, filepath, representative_data, use_h5=True):
        extension = os.path.splitext(filepath)[1]
        assert extension == EXTENSION_PB
        assert use_h5 == (self._model_h5 is not None)

        if use_h5 is not None:
            log_prefix = '%s %s' % (self._model.log_prefix(), ' [BatchN workaround]')
            # seems to be required or we get errors with BatchNormalization
            h5_filepath = self._model.filepath_h5()
            ...
            logger.debug('%s Creating TFLiteConverter from existing keras session...', self._model.log_prefix())
        ...
        logger.debug('%s Converting to tflite INT8 model...', self._model.log_prefix())
        try:
            tflite_model = converter.convert()
        except Exception as e:
            logger.exception('%s Caught exception while converting model to tflite INT8: %s',
                             self._model.log_prefix(), e)
            return ERROR_TF_META_CAUGHT_EXCEPTION
        tflite_filepath = self._model.filepath_tflite()
        logger.debug('%s Saving tflite model to %s...', self._model.log_prefix(), tflite_filepath)
